...
import com.microsoft.aad.adal4j.interactive.BrowserLauncher;
import com.microsoft.aad.adal4j.interactive.OAuthParameter;
import com.microsoft.aad.adal4j.interactive.OAuthResponseType;
import com.microsoft.aad.adal4j.interactive.PromptBehavior;
import com.microsoft.aad.adal4j.interactive.webserver.AADWebServer;
import com.microsoft.aad.adal4j.interactive.webserver.AuthCodeCallback;
import com.microsoft.aad.adal4j.interactive.webserver.ClosedCallback;
...
public class AuthenticationContext {
	 ...
	 * 			Name or ID of the client requesting the token.
	 * @param redirectUri
	 * 			URI to which the Azure AD interactive authentication sequence
	 * 			should redirect the browser client to with the authorization
	 * 			code. This URI should match exactly with the value that was
	 *			provided when the application was registered in the Azure AD
	 *			tenant.
	 * @param windowTitle
	 * 			The title text to be used in the popup window containing the
	 * 			browser.
	 * @param promptBehavior
	 * 			A value from the {@link PromptBehavior} enumeration. Passing
	 * 			{@link PromptBehavior.LOGIN} will cause the the authentication
	 * 			sequence to always present a login prompt. Passing
	 * 			{@link PromptBehavior.REFRESH_SESSION} will cause the
	 * 			authentication sequence to present a menu of credentials to
	 *			sign in with in case the user has multiple organizational or
	 *			Microsoft accounts. Passing {@link PromptBehavior.ATTEMPT_NONE}
	 *			will cause the authentication sequence to attempt reuse of
	 *			browser session cookies that were cached during a previous
	 *			authentication attempt instead of presenting a login prompt.
	 * @param callback
	 * 			optional callback object for non-blocking execution.
	 * @return	A {@link Future} object representing the
	 *          {@link AuthenticationResult} of the call. It contains Access
	...
	public Future<AuthenticationResult> acquireToken(final String resource,
			...
			final String windowTitle, final PromptBehavior promptBehavior,
			final AuthenticationCallback callback) throws IOException {
		if (StringHelper.isBlank(resource)) {
			throw new IllegalArgumentException("resource is null or empty");
		}
		...
		// acquire an auth code
		Futures.addCallback(acquireAuthCodeInteractiveAsync(
				resource, clientId,
				redirectUri, windowTitle,
				promptBehavior.toString()), new FutureCallback<String>() {
			@Override
			public void onSuccess(String authorizationCode) {
				Future<AuthenticationResult> tokenFuture = acquireTokenByAuthorizationCode(
						authorizationCode, resource, clientId, redirectUri, callback);
				ListenableFuture<AuthenticationResult> tokenListenableFuture =
						JdkFutureAdapters.listenInPoolThread(tokenFuture);

				Futures.addCallback(tokenListenableFuture, new FutureCallback<AuthenticationResult>() {
					@Override
	...
	private ListenableFuture<String> acquireAuthCodeInteractiveAsync(
		...
		try {
			...
			params.put(OAuthParameter.redirectUri, redirectUri.toString());
			...
			String query = null;
			query = URLUtils.serializeParameters(params);

			// build the actual URI
			String adUri = this.authenticationAuthority.getAuthorizeUri();
			adUri = adUri + "?" + query;
				...
				public void onAuthCodeReceived(String code, Map<String, String> params) {
					...
					try {
						...
						if (Strings.isNullOrEmpty(code)) {
							...
							future.setException(new AuthenticationException(msg));
						} else {
							future.set(code);
						}
					} finally {
